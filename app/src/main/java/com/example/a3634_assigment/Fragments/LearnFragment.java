...
public class LearnFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(SessionInfo.currentUser.getScore()>=100) {
            ...
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (200 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Earth!");

        }
        if(SessionInfo.currentUser.getScore()>=200) {
            planetAdapter.setData(PlanetBank.getLevel3());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (300 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Mars!");
        }
        if(SessionInfo.currentUser.getScore()>=300) {
            planetAdapter.setData(PlanetBank.getLevel4());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (400 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Jupiter!");

        }
        if(SessionInfo.currentUser.getScore()>=400) {
            planetAdapter.setData(PlanetBank.getLevel5());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (500 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Saturn!");

        }
        if(SessionInfo.currentUser.getScore()>=500) {
            planetAdapter.setData(PlanetBank.getLevel6());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (600 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Uranus!");

        }
        if(SessionInfo.currentUser.getScore()>=600) {
            planetAdapter.setData(PlanetBank.getLevel7());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (700 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Neptune!");

        }
        if(SessionInfo.currentUser.getScore()>=700) {
            planetAdapter.setData(PlanetBank.getLevel8());
            recyclerView.setAdapter(planetAdapter);
            levelUnlock.setText("Earn "+ (800 - SessionInfo.currentUser.getScore()) + " more points to unlock Level Pluto!");
